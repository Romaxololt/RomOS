...
def writeL(chaine, numero_ligne): ######################## N1
...
def configbasic():
    configlines()
    writeL("core.py,Ctone.py,GT8+.py,Log.txt,Logo.ico,RomaOSetup.py,ROS.py,Thor.py,Wath_ER.py,CryptROS.py", 1)
    writeL("ghp_,m579q,iMG1GD,POdUdp,9iM7G,QxYzk,U5v4,LFD,xz", 2)

def is_different(variable1, variable2):
...
def maj_system():
    ...
    for i in allfile:
        copy_file(i)
        with open(i, 'r', encoding='utf-8', errors='ignore') as fichier:
            contenu = fichier.read()
        if is_different(copy_file, contenu) == True:
            print(f"mise a jour de {i}")
            download_file(i)
