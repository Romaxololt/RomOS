...
def main():
    ...
    while running:
        ...
        if cmd.lower() == "exit": # quitter le systeme
            ...
            cmd = cmd.replace("ros ", '')
            if we.check_file_git(cmd) :print(f"téléchargement de {cmd}"),we.download_file(cmd)
...
-s (commande) : execute une commande\nclock : change la vitesse du script\nupdate : système de mise a jour\nros (nom du fichier) : telecharge un fichier
vrs (fichier) : dit la version actuelle d'un script et sa version la plus récente\nreboot : redémarre le système\nconfig : configure ROS""")
        else:
